...
foreach($cursor as $i=>$statistic) {
    $downloads[] = '["' . date('Y-m-d', $statistic['dlDate']->sec) . '", ' . $statistic['downloads'] . ']';
    $downloadsCompared[] = '["' . date('Y-m-d', $statistic['dlDate']->sec) . '", ' . $avgDownloads[$i] . ']';
}

$page->addScript('var statistics = [' . implode(',', $downloads) . '];');
$page->addScript('var statisticsCompared = [' . implode(',', $downloadsCompared) . '];');
